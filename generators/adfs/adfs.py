# Create a Federation deployment with ADFS and Active Directory
...
size_adfs = "Standard_D2as_v4"

# dc_ip - The domain controller IP address
dc_ip = ""

# adfs_ip - The ADFS IP address
adfs_ip = ""

# adfs_hostname - The ADFS hostname (You can change this here for your environment) 
adfs_hostname = "adfs"
...
# Add argument for Trusted, Signed Certificate for ADFS (Importing a trusted pfx file certificate) 
parser.add_argument('-tc', '--trusted_cert', dest='trusted_cert')
# Note:  When this is enabled, this points to the file name of the pfx file (Default is self-signed cert)

# Add argument for pfx file password (The password protected file storing Trusted Certificate) 
parser.add_argument('-pp', '--pfx_password', dest='pfx_password')
...
# Check if a trusted, signed certificate is going to be used for ADFS
# Note:  This is optional (By default the cert is self-signed)
trusted_cert = False
if args.trusted_cert:
    trusted_cert = True

# Check if a pfx password is specified
pfx_password = False 
if args.pfx_password:
    pfx_password = True

# If trusted_cert or pfx_password is specified, the other is required 
if trusted_cert and not pfx_password:
    print("[-] Error:  The pfx password (--pfx_password <password>) is required if trusted certificate is enabled")
    exit()
elif pfx_password and not trusted_cert: 
    print("[-] Error:  The trusted certificate (--trusted_cert <file>) is required if pfx_password is enabled")
    exit()
...
# Install sysmon on endpoints and ADFS, true by default
...
tmain_file = "main_adfs.tf"
tproviders_file = "providers.tf"
tnet_file = "network_adfs.tf"
tnsg_file = "nsg_adfs.tf"
tdc_file = "dc_adfs.tf"
tadfs_file = "adfs.tf"
tsentinel_file = "sentinel_adfs.tf"
tstorage_file = "storage_adfs.tf"
...
if dc_enable:
    if ad_vlan_count == 1:
        # This is the last octet of the dc's IP 
        last_octet = "4"
...
## get the adfs_ip
if ad_vlan_count == 1:
    # This is the last octet of the adfs's IP
    last_octet = "5"
    elements = ad_subnet_prefix.split('.')
    adfs_ip = elements[0] + "." + elements[1] + "." + elements[2] + "." + last_octet
else:
    print("[-] DC is enabled without a subnet assignment")
    print("[-] Set a type of ad_vlan to one of the subnets")
    exit()
...
data "template_file" "PS_TEMPLATE_VAR_NAME" {
  ...
  vars  = {
    ...
    ad_domain                 = "AD_DOMAIN" 
    storage_acct_s            = ADFS_STORAGE_ACCOUNT
    storage_container_s       = ADFS_STORAGE_CONTAINER
...
### This allows sysmon to be installed on endpoints
####
...
# Create a storage account for all files
resource "azurerm_storage_account" "storage_adfs" {
  name                     = "adfs${random_string.suffix.id}"
  resource_group_name = "${var.resource_group_name}-${random_string.suffix.id}"
  location                 = var.location
  account_tier             = "Standard"
  account_replication_type = "LRS"
...
# Create storage container for files
resource "azurerm_storage_container" "storage_adfs" {
  name                  = "provisioning"
  storage_account_name  = azurerm_storage_account.storage_adfs.name
  container_access_type = "blob"

  depends_on = [azurerm_resource_group.network]
}

# Upload the ADFS Domain Join script to storage account
resource "azurerm_storage_blob" "adfs_domain_join" {
  name                   = "domain_join.ps1"
  storage_account_name   = azurerm_storage_account.storage_adfs.name
  storage_container_name = azurerm_storage_container.storage_adfs.name
  type                   = "Block"
  source                 = "${path.module}/output/adfs/domain_join.ps1"

  depends_on = [local_file.adfs_domain_join]
}

# Upload the ADFS install script 
resource "azurerm_storage_blob" "install_adfs" {
  name                   = "install_adfs.ps1"
  storage_account_name   = azurerm_storage_account.storage_adfs.name
  storage_container_name = azurerm_storage_container.storage_adfs.name
  type                   = "Block"
  source                 = "${path.module}/output/adfs/install_adfs.ps1"

  depends_on = [local_file.install_adfs]
}
...
# build the ADFS storage tf file
# Always open the file, since it provides bootstrap scripts for ADFS

# Begin Sysmon template, in case it is needed (Enabled by default)
sysmon_template = '''
...
resource "azurerm_storage_blob" "sysmon_zip_sentinel" {
  ...
  storage_container_name = azurerm_storage_container.storage_adfs.name
  type                   = "Block"
  source                 = "${path.module}/../../shared/${local.sysmon_zip_s}"
}
'''
# End Sysmon template

# Trusted, signed certificate upload to storage account
trusted_cert_blob = '''
# Upload pfx file for Trusted, Signed Certificate 
resource "azurerm_storage_blob" "pfx_file_blob" {
  name                   = local.pfx_file_s
  storage_account_name   = azurerm_storage_account.storage_adfs.name
  storage_container_name = azurerm_storage_container.storage_adfs.name
  type                   = "Block"
  source                 = "${path.module}/${local.pfx_file_s}"
}
'''

# open tf for storage adfs
print("[+] Creating ADFS storage configuration file: ",tstorage_file)
storage_text_file = open(tstorage_file, "w")

# Get the storage template
storage_string = get_storage_template()

if install_sysmon_enabled:
    print("[+] Sysmon enabled - Adding sysmon configuration files for endpoints to use",tstorage_file)
    storage_string = storage_string.replace("SYSMON_TEMPLATE", sysmon_template)
else:
    storage_string = storage_string.replace("SYSMON_TEMPLATE", "")

if trusted_cert:
    print("[+] Trusted, signed certificate will be imported and used: ",args.trusted_cert)
    storage_string = storage_string.replace("PFX_FILE", args.trusted_cert)
    storage_string = storage_string.replace("TRUSTED_CERT_INFO", trusted_cert_blob)
else:
    storage_string = storage_string.replace("PFX_FILE", "")
    storage_string = storage_string.replace("TRUSTED_CERT_INFO", "")

# Finally write the file
n = storage_text_file.write(storage_string)
...
if (win10_count > 0):
    ...
    for i in range(win10_count):
        ...
        if install_sysmon_enabled == True:
            ...
            new_endpoint_template = new_endpoint_template.replace("ADFS_STORAGE_ACCOUNT", 'azurerm_storage_account.storage_adfs.name')
            # replace container name
            new_endpoint_template = new_endpoint_template.replace("ADFS_STORAGE_CONTAINER", 'azurerm_storage_container.storage_adfs.name')
            ...
            new_endpoint_template = new_endpoint_template.replace("ADFS_STORAGE_ACCOUNT", '""')
            new_endpoint_template = new_endpoint_template.replace("ADFS_STORAGE_CONTAINER", '""')
        ...
        if dc_enable and config_win10_endpoint['join_domain'].lower() == 'true':
...
### BEGIN ADFS CONFIGURATION
def get_adfs_template():
    template = '''

variable adfs_hostname {
  default = "ADFS_HOSTNAME"
  description = "The computer name for ADFS server"
}

variable adfs_offer {
  default = "WindowsServer"
  description = "The image reference OS offer"
}

variable adfs_sku {
  default = "2019-Datacenter"
  description = "The image reference OS sku"
}

variable adfs_version {
  default   = "latest"
  description = "The image reference OS version"
}

resource "azurerm_public_ip" "adfs_external" {
  name                     = "adfs-public-ip-${random_string.suffix.id}"
  location                = var.location
  resource_group_name = "${var.resource_group_name}-${random_string.suffix.id}"
  allocation_method       = "Static"
...
resource "azurerm_network_interface" "adfs-nic-int" {
  name                    = "adfs-int-nic-${random_string.suffix.id}"
  location                = var.location
  resource_group_name = "${var.resource_group_name}-${random_string.suffix.id}"
  internal_dns_name_label = var.adfs_hostname

  ip_configuration {
    name                          = "primary"
    subnet_id                     = azurerm_subnet.AD_SUBNET_NAME-subnet.id
    private_ip_address_allocation = "Static"
    private_ip_address            = "ADFS_IP_ADDRESS"
    public_ip_address_id          = azurerm_public_ip.adfs_external.id
  }
  depends_on = [azurerm_resource_group.network]
}

locals {
  virtual_machine_name_adfs = var.adfs_hostname
  virtual_machine_fqdn_adfs = "${local.virtual_machine_name_adfs}.AD_DOMAIN"
  custom_data_params_adfs   = "Param($RemoteHostName = \\"${local.virtual_machine_fqdn_adfs}\\", $ComputerName = \\"${local.virtual_machine_name_adfs}\\")"
  custom_data_adfs         = base64encode(join(" ", [local.custom_data_params_adfs, data.template_file.adfs_bootstrap_script.rendered ]))
}

data "template_file" "adfs_bootstrap_script" {
  template = file("${path.module}/files/adfs/bootstrap-adfs.ps1.tpl")

  vars  = {
    join_domain               = true ? 1 : 0 
    install_sysmon            = true ? 1 : 0 
    dc_ip                     = "DC_IP"
    winrm_username            = "WINRM_USERNAME"
    winrm_password            = "WINRM_PASSWORD"
    admin_username            = "ADMIN_USERNAME_ADFS"
    admin_password            = "ADMIN_PASSWORD_ADFS"
    ad_domain                 = "AD_DOMAIN"
    ...
    sysmon_config_s           = SYSMON_CONFIG
    sysmon_zip_s              = SYSMON_ZIP
    domain_join_s             = "domain_join.ps1"
    install_adfs_s            = "install_adfs.ps1"
    aadconnect_file           = var.azure_aadconnect_file
  }
}

resource "local_file" "adfs_debug_bootstrap_script" {
  content = data.template_file.adfs_bootstrap_script.rendered
  filename = "${path.module}/output/adfs/bootstrap-adfs.ps1"
}

resource "local_file" "adfs_domain_join" {
  content = templatefile("${path.module}/files/adfs/domain_join.ps1.tpl",
  {
    join_domain               = true ? 1 : 0
    dc_ip                     = "DC_IP"
    winrm_username            = "WINRM_USERNAME"
  ...
  filename = "${path.module}/output/adfs/domain_join.ps1"
}

resource "local_file" "install_adfs" {
  content = templatefile("${path.module}/files/adfs/install_adfs.ps1.tpl",
  {
    trusted_cert              = TRUSTED_CERT ? 1 : 0
    pfx_file                  = "PFX_FILE"
    pfx_password              = "PFX_PASSWORD"
    ad_domain                 = "AD_DOMAIN"
    hostname                  = var.adfs_hostname
    winrm_username            = "WINRM_USERNAME"
    winrm_password            = "WINRM_PASSWORD"
    admin_username            = "ADMIN_USERNAME_ADFS"
    admin_password            = "ADMIN_PASSWORD_ADFS"
  ...
  filename = "${path.module}/output/adfs/install_adfs.ps1"
}

resource "azurerm_windows_virtual_machine" "adfs" {
  name                          = var.adfs_hostname
  location                      = var.location
  resource_group_name = "${var.resource_group_name}-${random_string.suffix.id}"
  size                       = "SIZE_ADFS"
  computer_name  = var.adfs_hostname
  admin_username = "ADMIN_USERNAME_ADFS"
  admin_password = "ADMIN_PASSWORD_ADFS"
  custom_data    = local.custom_data_adfs

  network_interface_ids         = [
    azurerm_network_interface.adfs-nic-int.id,
  ]
  ...
  source_image_reference {
    ...
    offer     = var.adfs_offer 
    sku       = var.adfs_sku 
    version   = var.adfs_version 
  }

  additional_unattend_content {
      content      = "<AutoLogon><Password><Value>ADMIN_PASSWORD_ADFS</Value></Password><Enabled>true</Enabled><LogonCount>1</LogonCount><Username>ADMIN_USERNAME_ADFS</Username></AutoLogon>"
      setting = "AutoLogon"
  }

  additional_unattend_content {
      content      = file("${path.module}/files/adfs/FirstLogonCommands.xml")
      setting = "FirstLogonCommands"
  }
  depends_on = [
    azurerm_resource_group.network,
    azurerm_storage_blob.adfs_domain_join,
    azurerm_storage_blob.ad_connect_msi,
    azurerm_storage_blob.install_adfs
  ]
}

resource "azurerm_virtual_machine_extension" "azurerm_vm_adfs" {
  name                 = "OMSExtension"
...
ADFS Details
-------------------------
Computer Name:  ${azurerm_windows_virtual_machine.adfs.computer_name}
Private IP: ADFS_IP_ADDRESS 
Public IP:  ${azurerm_public_ip.adfs_external.ip_address}
local Admin:  ${azurerm_windows_virtual_machine.adfs.admin_username}
local password: ADMIN_PASSWORD_ADFS
Domain:  AD_DOMAIN
WinRM Username:  WINRM_USERNAME
WinRM Password:  WINRM_PASSWORD
Storage Container: ${azurerm_storage_container.storage_adfs.name}
--------------
ADFS Endpoint
--------------
Internal: https://${var.adfs_hostname}.AD_DOMAIN/adfs/ls/idpinitiatedsignon.aspx 
External: https://${azurerm_public_ip.adfs_external.ip_address}/adfs/ls/idpinitiatedsignon.aspx 
Note: For external please setup a DNS 'A' record resolving ${var.adfs_hostname}.AD_DOMAIN to ${azurerm_public_ip.adfs_external.ip_address}
EOS
}

'''
    return template
### END ADFS CONFIGURATION
...
resource "azurerm_storage_blob" "ad_connect_msi" {
  ...
  storage_container_name = azurerm_storage_container.storage-container.name
  type                   = "Block"
  source                 = "${path.module}/../../shared/${var.azure_aadconnect_file}"
}
...
###
# Beginning of adfs_sentinel.tf creation 
###

# open adfs_sentinel.tf
adfs_text_file = open(tadfs_file, "w")

# get the template
default_adfs_template = get_adfs_template()

# replace the ADFS IP Address
adfs_string = default_adfs_template.replace("ADFS_IP_ADDRESS", adfs_ip)

# replace the ADFS instance size
adfs_string = adfs_string.replace("SIZE_ADFS", size_adfs)

# replace the ADFS hostname 
adfs_string = adfs_string.replace("ADFS_HOSTNAME", adfs_hostname)
...
if args.user_csv:
    winrm_user = get_winrm_user(args.user_csv)
    winrm_username = winrm_user[0]
    adfs_string = adfs_string.replace("WINRM_USERNAME", winrm_username)
else:
    adfs_string = adfs_string.replace("WINRM_USERNAME", default_winrm_username)

# Trusted Certificate or Self signed
if trusted_cert:
    adfs_string = adfs_string.replace("TRUSTED_CERT", "true")
    adfs_string = adfs_string.replace("PFX_FILE", args.trusted_cert)
    adfs_string = adfs_string.replace("PFX_PASSWORD", args.pfx_password)
else:
    adfs_string = adfs_string.replace("TRUSTED_CERT", "false")
    adfs_string = adfs_string.replace("PFX_FILE", "")
    adfs_string = adfs_string.replace("PFX_PASSWORD", "")

# replace the DC_IP so ADFS can do domain join
adfs_string = adfs_string.replace("DC_IP", dc_ip)

# Replace WinRM Password
if args.user_csv:
    winrm_password = winrm_user[1]
    adfs_string = adfs_string.replace("WINRM_PASSWORD", winrm_password)
else:
    adfs_string = adfs_string.replace("WINRM_PASSWORD", default_winrm_password)

# replace the variable for local admin username 
adfs_string = adfs_string.replace("ADMIN_USERNAME_ADFS", default_admin_username)

# replace the variable for local admin password 
adfs_string = adfs_string.replace("ADMIN_PASSWORD_ADFS", default_admin_password)

# Replace the AD Domain
adfs_string = adfs_string.replace("AD_DOMAIN", default_domain)

# replace for install_sysmon_enabled
if install_sysmon_enabled == True:
    # replace storage account name
    adfs_string = adfs_string.replace("ADFS_STORAGE_ACCOUNT", 'azurerm_storage_account.storage_adfs.name')
    # replace container name
    adfs_string = adfs_string.replace("ADFS_STORAGE_CONTAINER", 'azurerm_storage_container.storage_adfs.name')
    # replace sysmon config
    adfs_string = adfs_string.replace("SYSMON_CONFIG", 'local.sysmon_config_s')
    # replace sysmon zip
    adfs_string = adfs_string.replace("SYSMON_ZIP", 'local.sysmon_zip_s')
    # replace sysmon_depends for endpoint
    adfs_string = adfs_string.replace("SYSMON_DEPENDS", sysmon_depends_string)
else:
    adfs_string = adfs_string.replace("ADFS_STORAGE_ACCOUNT", '""')
    adfs_string = adfs_string.replace("ADFS_STORAGE_CONTAINER", '""')
    adfs_string = adfs_string.replace("SYSMON_CONFIG", '""')
    adfs_string = adfs_string.replace("SYSMON_ZIP", '""')
    adfs_string = adfs_string.replace("SYSMON_DEPENDS", "")

# replace with AD subnet
adfs_string = adfs_string.replace("AD_SUBNET_NAME", ad_subnet_name)

# Write the adfs_sentinel.tf
print("[+] Creating the adfs terraform file: ", tadfs_file)
logging.info('[+] Creating the adfs terraform file: %s', tadfs_file)
print("  [+] ADFS Server Information:")
logging.info('  [+] ADFS Server Information')
print("  [+] Server Hostname: ",adfs_hostname)
logging.info('      [+] Server Hostname: %s', adfs_hostname)
adfs_fqdn = adfs_hostname + "." + default_domain 
print("  [+] FQDN: ",adfs_fqdn)
logging.info('      [+] FQDN: %s', adfs_fqdn)
adfs_endpoint = "https://" + adfs_fqdn + "/adfs/ls/idpinitiatedsignon.aspx" 
print("  [+] Signon Endpoint: ",adfs_endpoint)
logging.info('  [+] Signon Endpoint: %s', adfs_endpoint)
if trusted_cert:
    if os.path.exists(args.trusted_cert):
        print("  [+] Building with Trusted, Signed certificate: ",args.trusted_cert)
        logging.info('      [+] Building with trusted, signed certificate: %s', args.trusted_cert)
    else:
        print("[-] Error:  Certificate file not found: ",args.trusted_cert)
        exit()
else:
    print("  [+] Building with Self-Signed certificate")
    logging.info('    [+] Building with Self-Signed certificate')
n = adfs_text_file.write(adfs_string)
adfs_text_file.close()

###
# End of adfs_sentinel.tf creation 
###
...
###
# End of dc_sentinel.tf creation 
###
...
if args.ad_domain and dc_enable:
    default_domain = args.ad_domain
