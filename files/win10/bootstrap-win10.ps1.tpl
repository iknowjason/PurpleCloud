...
if ( $is -eq 1 ) {
  ...
  (New-Object System.Net.WebClient).DownloadFile('https://${storage_acct_s}.blob.core.windows.net/${storage_container_s}/${sysmon_zip}', 'C:\terraform\Sysmon.zip')
  ...
  if (-not(Test-Path -Path "C:\terraform\sysmonconfig-export.xml" -PathType Leaf)) {
    try {
      ...
      (New-Object System.Net.WebClient).DownloadFile('https://${storage_acct_s}.blob.core.windows.net/${storage_container_s}/${sysmon_config}', 'C:\terraform\sysmonconfig-export.xml')
    } catch {
...
if ( $ia -eq 1 ) {
  ...
  $uri =  'https://${storage_acct_v}.blob.core.windows.net/${storage_container_v}/${windows_msi}'
  lwrite("$mtime Downloading velociraptor client from $uri")

  $a = 0
  do {
    $mtime = Get-Date
    lwrite("$mtime iteration $a")
    (New-Object System.Net.WebClient).DownloadFile('https://${storage_acct_v}.blob.core.windows.net/${storage_container_v}/${windows_msi}', 'C:\terraform\${windows_msi}')
    $a++
    Start-Sleep -Seconds 5
  } until ($a -eq 5 -Or (Test-Path -Path "C:\terraform\${windows_msi}" -PathType Leaf))

  $mtime = Get-Date
  lwrite("$mtime Downloaded")
  ...
  lwrite("$mtime Download velociraptor client configuration")
  (New-Object System.Net.WebClient).DownloadFile('https://${storage_acct_v}.blob.core.windows.net/${storage_container_v}/${vclient_config}', 'C:\terraform\Velociraptor.config.yaml')

  # Copy the file
  Copy-Item "C:\terraform\Velociraptor.config.yaml" -Destination "C:\Program Files\velociraptor"
  ...
  do {
    $mtime = Get-Date
    lwrite("$mtime iteration $a")
    #msiexec.exe /I C:\terraform\${windows_msi} /quiet
  ...
  lwrite("$mtime Velociraptor service running")

  #lwrite("$mtime Restart velociraptor service")
  # re-start Velociraptor service
  #Restart-Service -name "velociraptor"

  # Download winlogbeat zip file with binary
  $mtime = Get-Date
  lwrite("$mtime Download Winlogbeat for HELK")
  (New-Object System.Net.WebClient).DownloadFile('https://${storage_acct_v}.blob.core.windows.net/${storage_container_v}/${winlogbeat_zip}', 'C:\terraform\winlogbeat.zip')

  # Download winlogbeat config yml
  $mtime = Get-Date
  if (-not(Test-Path -Path "C:\terraform\winlogbeat.yml" -PathType Leaf)) {
    try {
      lwrite("$mtime Download winlogbeat config")
      (New-Object System.Net.WebClient).DownloadFile('https://${storage_acct_v}.blob.core.windows.net/${storage_container_v}/${winlogbeat_config}', 'C:\terraform\winlogbeat.yml')
    } catch {
  ...
  Copy-Item "C:\terraform\winlogbeat.yml" -Destination "C:\terraform\Winlogbeat\winlogbeat-7.9.2-windows-x86_64"
  ...
  # Install thn Winlogbeat service 
  $mtime = Get-Date
