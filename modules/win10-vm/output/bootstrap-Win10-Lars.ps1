...
if ((gwmi win32_computersystem).partofdomain -eq $true) {
  ...
  ### Set the DA Username
  $username = "rtcfingroup".ToUpper() + "\" + "jason"
  ...
  if ( $op -Contains $winrm_check ){
    ...
    $DefaultDomainName = "rtcfingroup.com".ToUpper()
    $mtime = Get-Date
...
if ( $jd -eq 1 ) {
  ...
  Register-ScheduledTask WinlogonUser01 -Action $sta -Settings $sts -Trigger $stt -User "RTCAdmin" -Password "Password123"
} else {
